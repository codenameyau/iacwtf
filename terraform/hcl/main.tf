...
# resource "aws_s3_bucket" "kfc-bucket" {
#   bucket = "kfc-bucket"
# }

resource "aws_s3_bucket" "kfc-bucket-interpolation" {
  bucket = "${data.aws_caller_identity.current.account_id}-kfc-bucket-interpolation"
}

resource "aws_s3_bucket" "bitbucket" {
  bucket = "${data.aws_caller_identity.current.account_id}-bitbucket"
}

resource "aws_s3_bucket" "bitbucket2" {
    bucket = "${data.aws_caller_identity.current.account_id}-bitbucket2"
    tags = {
        dependency = aws_s3_bucket.bitbucket.arn
  }
}

resource "aws_s3_bucket" "bitbucket3" {
    bucket = "${data.aws_caller_identity.current.account_id}-bitbucket3"
    depends_on = [
        aws_s3_bucket.bitbucket2
    ]
}

output "bucket_names" {
  value = aws_s3_bucket.bitbucket[*].bucket
}
...
# # Interpolation
# resource "aws_s3_bucket" "bit-bucket-lol-interpolation" {
#   bucket = "${data.aws_caller_identity.current.account_id}-bucket-lol-interpolation"
# }

# resource "aws_s3_bucket" "bit-bucket-interpolation" {
#   bucket = "${data.aws_caller_identity.current.account_id}-bit-bucket-interpolation"
# }

# Dependencies
# resource "aws_s3_bucket" "bucket3" {
#     bucket = "${data.aws_caller_identity.current.account_id}-bit-bucket"
#     tags = {
#       dependency = aws_s3_bucket.bucket.
#   }
# }
